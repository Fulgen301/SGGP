...
protected func Connect(pEnd, pHome)
{
  ...
  if(pEnd->~IsStargate())
  {
	  pEnd->LocalN("pFrom") = this;
	  pEnd->SetAction("Income2");
  }
  else
  {
	  
	pEnd->SetAction("Warp", 0, pHome);
...
protected func Destruction()
{
  ...
  if(GetActionTarget()) 
  {
	  if(GetActionTarget()->~IsStargate())
	  {
		  GetActionTarget()->Deactivate();
	  }
	  else
	  {
		GetActionTarget()->RemoveObject();
	  }
  }
}
...
private func TransferWarpObject(pObj)
{
  ...
  if(GetActionTarget()->~IsStargate())
  {
	  if(GetActionTarget()->IsClose())
	  {
		  pObj->RemoveObject();
		  return 0;
	  }
	  else pObj->SetPosition(GetActionTarget()->GetX()+70, GetActionTarget()->GetY()+40);
  }
  else
