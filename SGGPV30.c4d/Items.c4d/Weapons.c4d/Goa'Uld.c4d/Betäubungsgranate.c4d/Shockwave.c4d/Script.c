...
protected func ScanAndRemove() {
  ...
  for(var obj in FindObjects( Find_Distance(iR/2+10), Find_NoContainer(), Find_Func("Zated") ))
  {
  ...
  for(var obj in FindObjects(Find_Distance(iR/2+10),Find_NoContainer(),Find_Func("IsMachine"),Find_Func("EMPShock")))
  {
  if(ObjectDistance(obj)<iR)
  {
  obj->EMPShock();
