...
func Precipitation()
{
  ...
  InsertMaterial(mat, RandomX(GetX(), GetX() + iLength) % LandscapeWidth() - GetX(), 0, vx, vy);
  InsertMaterial(mat, RandomX(GetX() + iLength / 3, GetX() + iLength * 2 / 3) % LandscapeWidth() - GetX(),10, vx, vy);
  InsertMaterial(mat, RandomX(GetX() - iLength / 3, GetX() + iLength * 4 / 3) % LandscapeWidth() - GetX(),10, vx, vy);
}
...
func Movement()
{
  ...
  if (GetX() > LandscapeWidth()-20) 
    SetPosition(25,GetY());
  if (GetX() < 20) 
    SetPosition(LandscapeWidth()-25,GetY());
}
