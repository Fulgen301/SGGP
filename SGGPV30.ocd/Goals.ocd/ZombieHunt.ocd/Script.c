...
public func IsFulfilled()
{
  ...
  if(!ObjectCount2(Find_Func("IsZombie")))
  {
  	 return(true);
  }
...
public func WinScoreChange(int iNewScore)
{
  ...
  if(!FindObject(CHOS))//Laufzeit?
    DoZombies();
}

public func DoZombies()
{
  var add = iWinScore*10-ObjectCount2(Find_Func("IsZombie"));
  
  if(add < 0)
  {
    while(add++)
    {
      var zombie = FindObject2(Find_Func("IsZombie"));
      Kill(zombie);//oder so...
    }
  }
  else
  {
    while(add--)
    {
      var zombie = PlaceAnimal(RandomIndex4K([ZCLK,ZCLK,ZCLK,ZCLK,ZCLK]));
...
public func Activate(iPlayer)
{
  ...
  if(IsFulfilled())
    ...
    text = Format("Es gibt noch %d Zombies!",ObjectCount2(Find_Func("IsZombie")));
    
  return(MessageWindow(text, iPlayer));
}
