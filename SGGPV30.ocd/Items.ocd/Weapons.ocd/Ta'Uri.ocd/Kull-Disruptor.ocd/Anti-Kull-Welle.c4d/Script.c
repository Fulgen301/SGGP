...
private func Traveling() {
  ...
  CreateParticle("EMPShock", 0,0, GetXDir(), GetYDir(), (Power/100) * RandomX(200,500) / 70, SetRGBaValue(Color(GetActTime())), this());

  //LaserLight(Power,RGB(255,255,255),RandomX(-Power/2,Power/2), RandomX(-Power/2,Power/2));

  Power++;
  if(Power == Weite) return(Remove());
...
private func Check()
{
  if(FindObject2(Find_Distance(25),Find_ID(KULL)))
  {
   FindObject2(Find_Distance(25),Find_ID(KULL))->Kill();
  }
  return(1);
}
